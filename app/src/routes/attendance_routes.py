...
def mark_attendance():
    ...
    try:
        ...
        # Parse QR data - format: "E101:Alice Smith" or just employee code/student ID
        ...
        if ':' in qr_data:
            ...
            if not student:
                return jsonify({
                    'success': False,
                    'message': f'Student ID {user_id} not found. Please check the QR code.'
                }), 200
        else:
            # Could be a student ID or employee code - check student table first
            user_id = qr_data
            student = db.get_student_by_id(user_id)
            
            if student:
                # Found in students table - use the student's name from database
                user_name = student['name']
            elif user_id in EMPLOYEES:
                # Found in employees
                user_name = EMPLOYEES.get(user_id)
            else:
                # Not found in either
                return jsonify({
                    'success': False,
                    'message': f'Invalid QR code or student number. No record found for {user_id}.'
                }), 200
